...
public class Move {
	...
	public void execute(Character c, Place charLoc, Move m) {
		...
		switch(mv) {
			...
			case LOOK:
				System.out.println(charLoc.description());
			...
			/*case DISPLAY:
				...
				break;*/
			case GET:
				Artifact pickup = charLoc.pickArtifact(arg, c);
				c.addUsrArtf(pickup);
				//canRepeat = false;
				break;
			
			case DROP:
				/*if(this.char_artf.size() == 0 && c instanceof Player) {
			...
			case CRAFT:
				...
				if(index >= 0) {
					...
					Artifact a = r.MakeArtifact(c);

					//a.print();
					if(a != null) {
						System.out.println("\n\t~ " + a.name() + " was crafted. It is now available in your inventory");
					}
				}
